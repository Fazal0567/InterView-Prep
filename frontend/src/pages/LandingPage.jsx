  ...
  return (
    <>
      <div className="w-full min-h-screen bg-[#FFFCEF] overflow-x-hidden relative">
        ...
        {/* Sticky Header */}
        <header className="sticky top-0 z-50 bg-[#FFFCEF] border-b border-amber-100 shadow-sm">
          <div className="container mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
            <div className="text-lg sm:text-xl font-bold text-black">Interview-Prep</div>
            <div className="md:hidden">
              <button onClick={() => setNavOpen(!navOpen)} className="text-black text-xl">
                ☰
              </button>
            </div>
            <nav className={`md:flex gap-6 items-center ${navOpen ? 'block' : 'hidden'} md:block`}>
              {user ? (
                <ProfileInfoCard />
              ) : (
                <button
        ...
        {/* Hero Section */}
        <section className="container mx-auto px-4 sm:px-6 pt-8 pb-6 md:pb-12 text-center md:text-left">
          <div className="flex flex-col items-center">
            <div className="flex items-center gap-2 text-xs sm:text-sm text-amber-600 font-semibold bg-amber-100 px-3 py-1 rounded-full border border-amber-300 mb-4">
              <LuSparkles size={16} /> AI Powered
            </div>

            <h1 className="text-[clamp(2rem,5vw,3.2rem)] font-bold text-black leading-tight mb-4">
            ...
            <p className="text-sm sm:text-base md:text-lg text-gray-800 mb-6 max-w-2xl">
              Get interview questions and model answers tailored to your role, experience, and target company — smart, sharp, and focused.
            </p>

            <button
              className="bg-black text-white text-sm sm:text-base font-semibold px-6 py-2.5 rounded-full transition hover:bg-yellow-100 hover:text-black hover:border-yellow-300 border border-transparent mb-8"
              onClick={handleCTA}
            >
              Get Started
            </button>
            ...
            <img
              ...
              className="w-full max-w-[90vw] md:max-w-[60%] rounded-xl shadow-lg"
            />
          </div>
        </section>
      </div>
